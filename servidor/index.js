  ...
  players.forEach(player => {
    if (!player.isJudge) {
      ...
      while (player.hand.length < 10 && room.whiteCardDeck.length > 0) {
        ...
        console.log('🃏 Carta branca adicionada à mão:', card, 'tipo:', typeof card);
      }
    }
...
io.on('connection', (socket) => {
  ...
  socket.on('join_room', (data) => {
    ...
    if (!room) {
      socket.emit('room_joined', { success: false, error: 'Sala não encontrada' });
      return;
    }
    
    if (room.gameState !== 'waiting') {
      socket.emit('room_joined', { success: false, error: 'Jogo já em andamento' });
      return;
    }
    ...
    if (!player) {
      socket.emit('room_joined', { success: false, error: 'Sala cheia ou erro interno' });
      return;
    }
    ...
    socket.emit('room_joined', {
      ...
      room: {
        id: roomCode,
        code: roomCode,
        name: `Sala de ${Array.from(room.players.values()).find(p => p.isHost)?.name || 'Host'}`,
        players: Array.from(room.players.values()),
        gameState: room.gameState,
        maxRounds: room.maxRounds
      },
  ...
  socket.on('rejoin_room', (data) => {
    ...
    if (existingPlayer) {
      ...
      console.log(`${playerName} reconectou à sala ${roomCode} - isHost: ${player.isHost}`);
    } else {
      console.log('Jogador não estava na sala, redirecionando para join normal');
      // Se não estava na sala, falhar
      socket.emit('room_joined', { success: false, error: 'Você não estava nesta sala' });
    }
  });
  ...
  socket.on('submit_card', (data) => {
    ...
    console.log('🎯 Carta submetida pelo jogador', roomPlayer.name + ':', submittedCard);
    ...
    const nonJudgePlayers = Array.from(room.players.values()).filter(p => !p.isJudge);
